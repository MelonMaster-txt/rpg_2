...
enum Archetype {
	BANDIT,     # Agressif, fort, skill_combat eleve
	WANDERER,   # Voyageur, curiosite, intelligence elevee
	MERCHANT,   # Marchand, charisme, skill_trading eleve
	HERMIT,     # Ermite, sagesse, intelligence tres elevee mais charisme bas
	FARMER,     # Paysan errant, skill_farming eleve
	WOODCUTTER  # Bucheron, force elevee, skill_woodcutting
}
...
const ARCHETYPE_DIALOGUE := {
	Archetype.BANDIT:     ["Qu'est-ce que tu veux ?", "Je n'ai peur de rien.", "Va-t-en avant de le regretter."],
	Archetype.WANDERER:   ["J'ai vu bien des choses sur la route...", "Le monde est vaste et plein de mysteres.", "Tu cherches quelque chose ?"],
	Archetype.MERCHANT:   ["Les affaires sont les affaires.", "J'ai vu de meilleures offres dans ma vie.", "Tu as l'air d'avoir besoin d'aide."],
	Archetype.HERMIT:     ["Laisse-moi tranquille.", "La solitude est une sagesse.", "..."],
	Archetype.FARMER:     ["La recolte a ete mauvaise cette annee.", "Je cherche une terre a cultiver.", "La vie est dure pour un paysan."],
	Archetype.WOODCUTTER: ["Ces arbres ne se couperont pas tout seuls.", "Le bois se fait rare par ici.", "Je connais chaque arbre de cette foret."]
}

@export var npc_name: String           = ""
@export var archetype: Archetype       = Archetype.WANDERER
@export var stats: CharacterStats      = null
@export var recruit_min_charisma: int  = 5   # Seuil charisme player pour recrutement
@export var recruit_min_force: int     = 0
@export var recruit_min_intel: int     = 0

# Revelees progressivement lors des discussions
...
static func generate_random() -> NpcData:
	...
	# On ajuste les stats selon l'archetype
...
func can_be_recruited_by_player() -> bool:
	return (
		GameManager.charisma >= recruit_min_charisma and
		GameManager.force    >= recruit_min_force    and
		GameManager.intelligence >= recruit_min_intel
	)

func get_reveal_info() -> String:
	# Revele une stat cachee a chaque conversation
	var all_revealable := ["force", "intelligence", "charisma", "stamina",
