...
func apply_appearance_data(data: Dictionary) -> void:
	...
	if data.has("outfit"):
		set_outfit(data["outfit"])
	if data.has("hair"):
		set_hair(data["hair"])
	if data.has("eye_style"):
		set_eye_style(data["eye_style"])
	...
	if data.has("eyes_color"):
		set_eyes_color(data["eyes_color"])
	if data.has("outfit_color"):
		set_outfit_color(data["outfit_color"])
