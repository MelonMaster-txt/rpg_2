# npc_spawner.gd — Autoload (stub de redirection)
# Le vrai NpcSpawner est dans game/characters/npcs/npc_spawner.gd
# Ce fichier existe pour satisfaire l'entrée Autoload qui pointe ici.
extends Node

const _REAL_SCENE_PATH: String = "res://game/characters/npcs/random_npc.tscn"
...
func spawn_random_around(origin: Vector2, radius: float, count: int = 1) -> void:
	...
	if scene_root == null:
		return
	var packed: PackedScene = load(_REAL_SCENE_PATH)
	if packed == null:
		push_error("NpcSpawner: scene introuvable -> " + _REAL_SCENE_PATH)
		return
