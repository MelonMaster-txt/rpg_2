# npc_inventory_ui.gd
# UI pour équiper/désiquiper les items d'un NPC.
# S'ouvre depuis le panneau de gestion du royaume.
# Usage : npc_inventory_ui.open(entry) où entry est un dict du CompanionManager.
extends CanvasLayer
...
# Items de base disponibles (à terme : viendra du GameManager.inventory ou d'un ItemDB)
...
func open(entry: Dictionary) -> void:
	_entry = entry
	# Récupère ou crée l'inventaire du NPC
...
func _build_slots() -> void:
	...
	for slot in NpcInventory.SLOTS:
		...
		var equipped_item = _inventory.equipped.get(slot)
	...
	# Afficher les bonus totaux
	var sep := HSeparator.new()
	slots_vbox.add_child(sep)
	var bonus_label := Label.new()
	var bonuses := _inventory.get_total_bonuses()
	if bonuses.is_empty():
...
func _open_equip_picker(slot: String) -> void:
	var items_for_slot: Array = AVAILABLE_ITEMS.filter(func(i): return i["slot"] == slot)
	if items_for_slot.is_empty():
		return
	# Fenêtre de choix simple
	...
	for item in items_for_slot:
		...
		var bonuses_str: String = ", ".join(
			item["bonuses"].keys().map(func(k): return "+%d %s" % [item["bonuses"][k], k])
		)
