...
const GROWTH_TIME_WATERED := 15.0
var _growth_timer: float = 0.0
var _growth_limit: float = GROWTH_TIME
...
func _process(delta: float) -> void:
	if _state == State.PLANTED or _state == State.WATERED:
		_growth_timer += delta
		if _growth_timer >= _growth_limit:
			_state = State.READY
			_refresh_visuals()
...
func _plant() -> void:
	...
	for crop in ["berries", "wheat", "herb"]:
		...
		if inv.get(seed_key, 0) > 0:
			chosen = crop
			_consume_seed(seed_key)
			break
	if chosen == "": chosen = "berries"
	_crop  = chosen
	_state = State.PLANTED
	_growth_timer = 0.0
	_growth_limit = GROWTH_TIME
	_refresh_visuals()
...
func _water() -> void:
	...
	_growth_timer = 0.0
	_growth_limit = GROWTH_TIME_WATERED
	_refresh_visuals()
...
func _harvest() -> void:
	...
	if chests.size() > 0:
		...
		for resource in yields:
			if chest.has_method("deposit"): chest.deposit(resource, yields[resource])
	else:
		for resource in yields:
			if GameManager.has_method("add_item"): GameManager.add_item(resource, yields[resource])
...
func _get_player_inventory() -> Dictionary:
	var chests: Array = get_tree().get_nodes_in_group("chest")
	if chests.size() > 0 and chests[0].get("inventory") != null:
		return chests[0].inventory
	return {}


func _consume_seed(seed_key: String) -> void:
	var chests: Array = get_tree().get_nodes_in_group("chest")
	if chests.size() > 0 and chests[0].get("inventory") != null:
		var chest = chests[0]
