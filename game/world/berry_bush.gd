...
# Frame 3 : plein + vide | Frame 4 : plein + quelques baies | Frame 5 : plein + plein de baies
extends Node2D
...
const FRAME_MAP: Dictionary = {
	...
	GrowthStage.HALF:   1,
	GrowthStage.FULL:   2,  # override par berry_stage quand FULL
}
...
# --- Exports ---
@export var growth_time: float         = 30.0   # secondes pour passer d'un stade de croissance
@export var berry_regrow_time: float   = 45.0   # secondes pour repousser les baies
@export var berry_yield_min: int       = 1
@export var berry_yield_max: int       = 4
@export var start_fully_grown: bool    = false   # true pour les buissons déjà adultes
...
func harvest() -> void:
	if _growth_stage != GrowthStage.FULL:
		return
	if _berry_stage == BerryStage.EMPTY:
		return
...
func _setup_frames() -> void:
	# Configure le SpriteFrames dynamiquement depuis le sprite sheet
	if not is_instance_valid(_sprite):
		return
