...
var _spawn_center:     Vector2i = Vector2i(0, 0)   # centre des spawn chunks
...
func _process(_delta: float) -> void:
	# Appliquer les chunks générés par le thread
	_flush_ready_queue()
...
func _generation_thread_loop() -> void:
	while _thread_running:
		...
		if coords == null:
			OS.delay_msec(4)  # sleep court si rien à faire
			continue

		# Calculs lourds ici (bruit, type, objets) — jamais de scene tree
...
func _compute_chunk_data(coords: Vector2i) -> Dictionary:
	## Toute la logique de génération (bruit, placement d'objets, type de biome)
	## se fait ici, dans le thread secondaire.
	var ctype: String = ChunkGenerator.get_chunk_type(coords)
	return {"type": ctype}


# ---------------------------------------------------------------------------
# MAIN THREAD — application des données générées + gestion du pool
# ---------------------------------------------------------------------------
func _flush_ready_queue() -> void:
	...
	for item in batch:
		...
		# Le chunk a peut-être été annulé pendant qu'on calculait
...
func _apply_ticket(coords: Vector2i) -> void:
	...
	match _ticket_levels[coords]:
		...
		_:
			pass  # SLEEP → dans le pool, pas dans la scène


# ---------------------------------------------------------------------------
# POOL — réutilisation des nodes
# ---------------------------------------------------------------------------
...
func _update_chunks() -> void:
	...
	# Calculer les zones souhaitées
	var active_set:  Dictionary = {}
	var lazy_set:    Dictionary = {}

	# Spawn chunks — toujours ACTIVE, ignorent le déchargement
	...
	# Charger / mettre à jour les tickets
	for c in active_set:
		if not _loaded_chunks.has(c):
			_queue_load_chunk(c, TicketLevel.ACTIVE)
		else:
	...
	# Décharger les chunks hors des deux zones (sauf spawn chunks)
	var to_unload: Array = []
...
func _queue_load_chunk(coords: Vector2i, level: TicketLevel) -> void:
	# Réserver la place tout de suite pour éviter les doublons
...
func _unload_chunk(coords: Vector2i) -> void:
	# Les spawn chunks ne sont jamais déchargés
...
func _draw() -> void:
	...
	for coords in _loaded_chunks.keys():
		...
		var lvl: TicketLevel = _ticket_levels.get(coords, TicketLevel.SLEEP)
		var col: Color
		match lvl:
